...
var parseFieldSdl = function parseFieldSdl(sdl) {
  return sdl ? (0, _graphql.parse)("type Type { ".concat(sdl, " }")).definitions[0].fields[0] : {};
};

exports.parseFieldSdl = parseFieldSdl;

var parseInputFieldSdl = function parseInputFieldSdl(sdl) {
  return sdl ? (0, _graphql.parse)("input Type { ".concat(sdl, " }")).definitions[0].fields : {};
};
...
var buildInputValueDefinitions = function buildInputValueDefinitions(fields) {
  ...
  if ((0, _isArray["default"])(fields)) {
    fields = fields.join('\n');
    arr = fields ? (0, _graphql.parse)("type Type { ".concat(fields, " }")).definitions[0].fields : [];
...
function paramsToString(params, cypherParams) {
  if (params.length > 0) {
    var strings = _lodash["default"].map(params, function (param) {
      return "".concat(param.key, ":").concat(param.paramKey ? "$".concat(param.paramKey, ".") : '$').concat(typeof param.value.index === 'undefined' ? param.key : "".concat(param.value.index, "_").concat(param.key));
    });
...
var getAdditionalLabels = function getAdditionalLabels(schemaType, cypherParams) {
  ...
  var _ref11 = labelDirective ? parseArgs(labelDirective.arguments) : {
    labels: []
  },
        ...
        if (isTemporalInputType(fieldType.name.value)) {
          ...
          if (temporalFunction) {
            ...
            if (formatted) {
              ...
              acc.push("".concat(paramName, ": ").concat(temporalFunction, "($").concat(paramKey ? "".concat(paramKey, ".") : '').concat(paramName, ")"));
            } else {
              (function () {
                var temporalParam = {};
                ...
                if ((0, _isArray["default"])(param)) {
                  (function () {
                    ...
                    acc.push("".concat(paramName, ": [value IN $").concat(paramKey ? "".concat(paramKey, ".") : '').concat(paramName, " | ").concat(temporalFunction, "(value)]"));
                  })();
                } else {
          ...
          // normal case
          acc.push("".concat(paramName, ":$").concat(paramKey ? "".concat(paramKey, ".") : '').concat(paramName));
        }
      }
...
var addDirectiveDeclarations = function addDirectiveDeclarations(typeMap, config) {
  ...
  typeMap['additionalLabels'] = (0, _graphql.parse)("directive @additionalLabels(labels: [String]) on OBJECT").definitions[0]; // TODO should we change these system directives to having a '_Neo4j' prefix
...
var getRelationMutationPayloadFieldsFromAst = function getRelationMutationPayloadFieldsFromAst(relatedAstNode) {
  var isList = false;
  var fieldName = '';
  return relatedAstNode.fields.reduce(function (acc, t) {
    ...
    if (fieldName !== 'to' && fieldName !== 'from') {
      ...
      acc.push("".concat(fieldName, ": ").concat(isList ? '[' : '').concat(_getNamedType(t).name.value).concat(isList ? "]" : '').concat((0, _graphql.print)(t.directives)));
    }
...
var initializeMutationParams = function initializeMutationParams(_ref15) {
  ...
  return (isCreateMutation(resolveInfo) || isUpdateMutation(resolveInfo)) && !mutationTypeCypherDirective ? _objectSpread({
    params: otherParams
  }, {
    first: first,
    offset: offset
...
var getOuterSkipLimit = function getOuterSkipLimit(first, offset) {
  return "".concat(offset > 0 ? " SKIP $offset" : '').concat(first > -1 ? ' LIMIT $first' : '');
};
...
var filterNullParams = function filterNullParams(_ref16) {
  ...
  return (0, _entries["default"])(_objectSpread({}, {
    offset: offset,
    first: first
  }, {}, otherParams)).reduce(function (_ref17, _ref18) {
...
var temporalPredicateClauses = function temporalPredicateClauses(filters, variableName, temporalArgs, parentParam) {
  return temporalArgs.reduce(function (acc, t) {
    ...
    if (temporalParam) {
      ...
      if (temporalParam['formatted']) {
        ...
        acc.push("".concat(variableName, ".").concat(argName, " = ").concat(getTemporalCypherConstructor(t), "($").concat( // use index if provided, for nested arguments
...
var getExcludedTypes = function getExcludedTypes(config, rootType) {
  return config && rootType && config[rootType] && (0, _typeof2["default"])(config[rootType]) === 'object' && config[rootType].exclude ? config[rootType].exclude : [];
};

exports.getExcludedTypes = getExcludedTypes;

var possiblyAddIgnoreDirective = function possiblyAddIgnoreDirective(astNode, typeMap, resolvers, config) {
  ...
  return fields.map(function (field) {
    ...
    if ( // has a custom resolver but not a directive
    getCustomFieldResolver(astNode, field, resolvers) && !getFieldDirective(field, 'neo4j_ignore') && // fields that behave in ways specific to the neo4j mapping do not recieve ignore
...
var getCustomFieldResolver = function getCustomFieldResolver(astNode, field, resolvers) {
