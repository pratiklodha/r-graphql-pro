...
function buildCypherSelection(_ref) {
  ...
  var recurse = function recurse(args) {
    ...
    return [subSelection, _objectSpread({}, shallowFilterParams, {}, subFilterParams)];
  };

  if (selections.find(function (_ref4) {
    var kind = _ref4.kind;
    return kind && kind === 'InlineFragment';
  })) {
    return selections.filter(function (_ref5) {
      var kind = _ref5.kind;
      return kind && kind === 'InlineFragment';
    }).reduce(function (query, selection, index) {
      var fragmentSelections = selection.selectionSet.selections;
  ...
  if (customCypher) {
    ...
    selection = recurse((0, _translate.temporalType)(_objectSpread({
      schemaType: schemaType,
      schemaTypeRelation: schemaTypeRelation,
      parentSelectionInfo: parentSelectionInfo
    }, fieldInfo)));
    ...
    var temporalClauses = (0, _utils.temporalPredicateClauses)(filterParams, nestedVariable, temporalArgs); // translate field, arguments and argument params
