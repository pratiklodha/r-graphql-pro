...
var possiblyAddDirectiveDeclarations = function possiblyAddDirectiveDeclarations(typeMap, config) {
...
var possiblyAddDirectiveImplementations = function possiblyAddDirectiveImplementations(schemaDirectives, typeMap, config) {
  if (shouldAddAuthDirective(config, 'isAuthenticated')) {
  ...
  if (shouldAddAuthDirective(config, 'hasScope')) {
    if (entityType === 'node') {
      if (operationType === 'Create' || operationType === 'Read' || operationType === 'Update' || operationType === 'Delete') {
        return (0, _utils.parseDirectiveSdl)("@hasScope(scopes: [\"".concat(typeName, ": ").concat(operationType, "\"])"));
      }
    }

    if (entityType === 'relation') {
      if (operationType === 'Add') operationType = 'Create';else if (operationType === 'Remove') operationType = 'Delete';
      return "@hasScope(scopes: [\"".concat(typeName, ": ").concat(operationType, "\", \"").concat(relatedTypeName, ": ").concat(operationType, "\"])");
    }
  }
